...
def _validate_ohlc_data(df: pd.DataFrame) -> int:
    ...
    try:
        ...
        if invalid_count > 0:
            ...
            df.loc[invalid_conditions, "high"] = df.loc[
                invalid_conditions, ["open", "high", "low", "close"]
            ].max(axis=1)

            # Low should be the minimum of all values
            df.loc[invalid_conditions, "low"] = df.loc[
                invalid_conditions, ["open", "high", "low", "close"]
            ].min(axis=1)
    ...
    try:
        ...
        for period in ta_config["sma_periods"]:
            if len(df) >= period:
                indicators[f"sma_{period}"] = (
                    ta.trend.SMAIndicator(df["close"], window=period).sma_indicator().iloc[-1]
                )

        for period in ta_config["ema_periods"]:
            if len(df) >= period:
                indicators[f"ema_{period}"] = (
                    ta.trend.EMAIndicator(df["close"], window=period).ema_indicator().iloc[-1]
                )
        ...
        if len(df) >= ta_config["bb_period"]:
            bb_indicator = ta.volatility.BollingerBands(
                df["close"], window=ta_config["bb_period"], window_dev=ta_config["bb_std"]
            )
        ...
        if len(df) >= 14:
            ...
            indicators["volume_sma"] = (
                ta.trend.SMAIndicator(df["volume"], window=min(20, len(df))).sma_indicator().iloc[-1]
            )
        ...
        if "keltner_upper" in indicators and "keltner_lower" in indicators:
            indicators["keltner_position"] = (current_price - indicators["keltner_lower"]) / (
...
def _create_technical_chart(df: pd.DataFrame, indicators: Dict[str, Any], crypto_name: str) -> bool:
    ...
    try:
        ...
        price_change = (
            ((current_price - chart_df["close"].iloc[0]) / chart_df["close"].iloc[0] * 100)
            if len(chart_df) > 1
            else 0
        )
        ...
        # Professional title
        enhanced_title = (
            f"{crypto_name.title()} - Professional Technical Analysis\n"
        ...
        if "volume" in chart_df.columns and (chart_df["volume"] > 0).any():
            try:
                ...
                volume_colors = [
                    bull_vol_color if row["close"] >= row["open"] else bear_vol_color
        ...
        try:
            temp_file = tempfile.NamedTemporaryFile(
                delete=False, suffix=".png", prefix=f"chart_{crypto_name}_"
            )
            fig.savefig(
                temp_file.name,
                format="png",
...
def _create_fallback_line_chart(df: pd.DataFrame, indicators: Dict[str, Any], crypto_name: str) -> bool:
    ...
    try:
        ...
        current_price = df["close"].iloc[-1]
        price_change = (
            ((current_price - df["close"].iloc[0]) / df["close"].iloc[0] * 100) if len(df) > 1 else 0
        )
        rsi_value = indicators.get("rsi", 0)
        ...
        if len(df) >= 20:
            try:
                ...
                ax1.plot(
                    df["datetime"],
                    sma_20,
                    color=colors.get("sma_20", "#FFD700"),
                    linewidth=2,
                    alpha=0.8,
                    label="SMA 20",
                )
        ...
        if len(df) >= 50:
            try:
                ...
                ax1.plot(
                    df["datetime"],
                    sma_50,
                    color=colors.get("sma_50", "#DA70D6"),
                    linewidth=2,
                    alpha=0.8,
                    label="SMA 50",
                )
        ...
        ax1.set_title(
            f"{crypto_name.title()} - Professional Technical Analysis (Fallback Line Chart)\n"
        ...
        if "rsi" in indicators and len(df) >= 14:
            try:
                ...
                if not rsi_series.empty and not rsi_series.isna().all():
                    ax2.plot(
                        df["datetime"][-len(rsi_series):],
                        rsi_series,
                        color=colors.get("rsi_line", "#F59E0B"),
                        linewidth=3,
                        label="RSI",
                    )
                    ax2.axhline(
                        y=70, color=colors.get("rsi_overbought", "#DC2626"), linestyle="--", alpha=0.8, linewidth=1
                    )
                    ax2.axhline(
                        y=30, color=colors.get("rsi_oversold", "#059669"), linestyle="--", alpha=0.8, linewidth=1
                    )
        ...
        try:
            temp_file = tempfile.NamedTemporaryFile(
                delete=False, suffix=".png", prefix=f"fallback_chart_{crypto_name}_"
            )
            fig.savefig(
                temp_file.name,
                format="png",
...
def _identify_candlestick_patterns(df: pd.DataFrame, forecast_horizon: str) -> List[str]:
    ...
    try:
        ...
        horizon_context = (
            ...
            if "day" in forecast_horizon.lower()
            else "long-term"
        )
        ...
        for i in range(len(recent_df)):
            ...
            # Hammer patterns
                ...
                row["lower_shadow_ratio"] > 0.5
                and row["upper_shadow_ratio"] < 0.1
            ...
            # Shooting star patterns
                ...
                row["upper_shadow_ratio"] > 0.5
                and row["lower_shadow_ratio"] < 0.1
                and row["close"] < row["open"]
            ):
        ...
        if len(recent_df) >= 3:
            for i in range(2, len(recent_df)):
                ...
                if (
                    ...
                    and second["body_ratio"] < 0.3  # Second candle small body (star)
                    and third["close"] > third["open"]  # Third candle bullish
                ...
                # Evening star pattern
                    ...
                    and second["body_ratio"] < 0.3  # Second candle small body (star)
                    and third["close"] < third["open"]  # Third candle bearish
                    and third["close"] < (first["open"] + first["close"]) / 2
...
def _generate_summary(
    ...
    summary_parts.extend(["", "**Technical Indicators Analysis:**"])
    ...
    summary_parts.extend(["", f"**Key Levels to Watch ({forecast_horizon}):**"])
...
def _create_enhanced_technical_chart(
    df: pd.DataFrame,
    indicators: Dict[str, Any],
    patterns: List[str],
    crypto_name: str,
    ...
    try:
        ...
        enhanced_title = (
            f"{crypto_name.title()} - Enhanced Technical Analysis ({forecast_horizon} Forecast)\n"
        ...
        if min_data_points >= 14:
            try:
                ...
                if not rsi.empty and not rsi.isna().all():
                    ...
                    apd.append(
                        mpf.make_addplot(
                            rsi, panel=1, color=colors.get("rsi_line", "#F59E0B"), width=3, ylabel="RSI (14)"
                        )
                    )

                    # RSI levels
                    rsi_levels = [(80, "#DC2626", 0.7), (70, "#DC2626", 0.9), (50, "#6B7280", 0.6), (30, "#059669", 0.9), (20, "#059669", 0.7)]

                    for level, color, alpha in rsi_levels:
                        rsi_level = pd.Series([level] * len(rsi), index=rsi.index)
                        apd.append(
                            mpf.make_addplot(
                                rsi_level, panel=1, color=color, width=1.5, linestyle="--", alpha=alpha
                            )
                        )
            ...
            try:
                ...
                if not macd_line.empty and not macd_signal_line.empty and not macd_histogram.empty:
                    ...
                    apd.extend(
                        [
                            mpf.make_addplot(
                                macd_line, panel=2, color=colors.get("macd_line", "#00D4AA"), width=3, ylabel="MACD (12,26,9)"
                            ),
                            mpf.make_addplot(
                                macd_signal_line, panel=2, color=colors.get("macd_signal", "#FF6B6B"), width=3
                            ),
                        ]
                    )
                    ...
                    apd.append(
                        mpf.make_addplot(
                            macd_histogram, panel=2, type="bar", color=macd_hist_colors, alpha=0.7, width=1.0
                        )
                    )
                    ...
                    apd.append(
                        mpf.make_addplot(
                            macd_zero, panel=2, color="#6B7280", width=1.5, linestyle="-", alpha=0.6
                        )
                    )
        ...
        try:
            temp_file = tempfile.NamedTemporaryFile(
                delete=False, suffix=".png", prefix=f"enhanced_chart_{crypto_name}_"
            )
            fig.savefig(
                temp_file.name,
                format="png",
    ...
    try:
        ...
        if patterns:
            ...
            main_ax.text(
                ...
                bbox=dict(boxstyle="round,pad=0.5", facecolor="#2A2E39", edgecolor="#363A45", alpha=0.9),
                color="#D1D4DC",
                fontweight="bold",
            )
